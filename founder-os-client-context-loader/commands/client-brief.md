...
1. Read the relationship-summary skill at `${CLAUDE_PLUGIN_ROOT}/skills/relationship-summary/SKILL.md` for the executive brief template, sentiment scoring, engagement metrics, risk flags, and health score formula.
2. Read the client-context skill at `${CLAUDE_PLUGIN_ROOT}/skills/client-context/SKILL.md` for CRM schema and data source hierarchy.
3. **Check dossier cache**:
   ...
   - If Dossier Stale is true or Dossier Generated At is older than 24 hours: warn the user and suggest running `/client:load [name]` first to refresh data. Still generate a brief from stale data with a staleness warning.
   - If no Dossier property is populated or no matching page found: inform the user that no dossier exists for this client. Suggest running `/client:load [name]` first. If the user wants to proceed anyway, perform a lightweight CRM-only lookup and generate a partial brief.
...
---
*Brief generated from [source description]. Run `/client:load [name]` to refresh.*
```
...
- **No cached dossier found**: Inform user and suggest `/client:load [name]`. Offer to generate a partial brief from CRM-only data.
...
- **Client not found in CRM or cache**: Report that no data exists for this client. Suggest checking the spelling or running `/client:load [name]` with the correct name.
...
```
/client:brief Acme Corp          # Generate 1-page brief from cached dossier
/client:brief "Acme Corp Inc"    # Quoted name for exact match
```
