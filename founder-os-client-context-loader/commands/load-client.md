...
1. Read the client-context skill at `${CLAUDE_PLUGIN_ROOT}/skills/client-context/SKILL.md` for CRM schema, lookup strategies, extraction rules, and completeness scoring.
2. Read the relationship-summary skill at `${CLAUDE_PLUGIN_ROOT}/skills/relationship-summary/SKILL.md` for health scoring and sentiment analysis.
...
```
/client:load Acme Corp                  # Default: CRM + email summary
/client:load Acme Corp --team           # Full 6-agent pipeline
/client:load Acme Corp --refresh        # Bypass cache, fresh data
/client:load "Acme Corp Inc" --hours=48 # Specific name, 48h lookback
/client:load Acme --team --refresh      # Full pipeline, no cache
```
