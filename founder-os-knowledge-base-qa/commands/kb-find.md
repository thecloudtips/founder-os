...
Read the knowledge-retrieval skill at `${CLAUDE_PLUGIN_ROOT}/skills/knowledge-retrieval/SKILL.md` for multi-source search pipeline, query formulation, relevance scoring, and preview generation.
...
- `[topic]` (required) -- the search topic or question. This is the positional argument before any flags. If missing or empty, prompt the user: "What topic would you like to search for? Usage: `/kb:find [topic]`" and stop.
...
*Run `/kb:ask [topic]` for a sourced answer · Knowledge Base Q&A*
```
...
No documents found matching "[topic]". Try broader search terms or check that sources are indexed with `/kb:index`.
```
...
This command does NOT save results to Notion. It answers "what documents do we have about X" -- ephemeral by design. For tracked Q&A with sourced answers, use `/kb:ask`.
...
Then run `/kb:find` again, or use `--sources=notion` to search Notion instead."
...
```
/kb:find onboarding
/kb:find "deployment process" --type=process
/kb:find pricing --sources=notion --limit=5
/kb:find API documentation --type=reference --sources=all --limit=20
/kb:find quarterly review --type=meeting-notes --limit=3
```
