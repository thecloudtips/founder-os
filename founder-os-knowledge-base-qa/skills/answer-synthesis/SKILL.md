...
This skill handles answer construction, citation formatting, confidence assessment, and conflict reconciliation only. Source retrieval, indexing, and search are the responsibility of the knowledge-retrieval skill. The command layer (`/kb:ask`) orchestrates the handoff between retrieval and synthesis.
...
Use numbered inline references and a citation block. Every answer must include at least one citation. See `${CLAUDE_PLUGIN_ROOT}/skills/answer-synthesis/references/citation-formats.md` for full format specification, examples, and edge cases.
...
For detailed scoring criteria with worked examples, see `${CLAUDE_PLUGIN_ROOT}/skills/answer-synthesis/references/citation-formats.md`.
...
- **Notion unavailable**: Cannot retrieve sources. Report: "Knowledge base search is unavailable -- Notion MCP server is not connected." Do not attempt to answer from memory.
