...
4. **Content**: Retrieve page content via `notion-fetch` for classification and keyword extraction. Limit content retrieval to the first 3000 characters to control API usage.
...
1. **Title pattern matching**: Match the source title against known patterns for each type. This is the fastest and most reliable signal. See `${CLAUDE_PLUGIN_ROOT}/skills/source-indexing/references/content-classification.md` for the full pattern list.
...
Extract 5-8 topic keywords from the source content to enable keyword-based search in the `/kb:ask` and `/kb:find` commands.
...
This ensures re-running `/kb:index` updates existing entries and adds new sources without creating duplicates.
...
- Warn: "Workspace contains more than 500 pages. Indexed the 500 most recently edited. Run /kb:index with parent filters to index additional sections."
...
`${CLAUDE_PLUGIN_ROOT}/skills/source-indexing/references/content-classification.md`
