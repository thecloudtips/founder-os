...
Search Notion and Google Drive to find relevant documents, score them for relevance, and extract content for answer synthesis or document listing. This skill powers both `/kb:ask` (full answer with citations) and `/kb:find` (ranked document listing with previews).
...
7. **Preview generation** -- produce 150-char previews for `/kb:find` output.
...
Consult `${CLAUDE_PLUGIN_ROOT}/skills/knowledge-retrieval/references/search-strategies.md` for detailed query variant examples and source-specific search tips.
...
**Search execution:**
1. Call `notion-search` with each query variant.
...
Consult `${CLAUDE_PLUGIN_ROOT}/skills/knowledge-retrieval/references/search-strategies.md` for scoring formula details, worked examples, and edge case adjustments.

## Content Extraction

For top-scoring results (top 5 for `/kb:ask`, top 10 for `/kb:find`), extract the most relevant content section.
...
For `/kb:find` results, generate a 150-character preview of each document.
...
search_results:
  ...
  results:
    - rank: 1
      ...
      preview: [150-char preview for /kb:find]
      extracted_content: [up to 3000 chars for /kb:ask]
      score_breakdown:
