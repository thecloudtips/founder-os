...
Load a pre-written project brief from a local file or Notion page, then generate a Statement of Work with 3 scope options (Conservative, Balanced, Ambitious). This is the "bring your own brief" mode — it skips the interactive discovery interview used by `/sow:generate`.
...
1. Check if the Notion MCP is available. If not, halt with: "Notion MCP is required to load Notion pages. See INSTALL.md for setup instructions."
2. Call `notion-fetch` with the full URL.
...
1. Read all 3 skills:
   - `${CLAUDE_PLUGIN_ROOT}/skills/scope-definition/SKILL.md`
   - `${CLAUDE_PLUGIN_ROOT}/skills/sow-writing/SKILL.md`
   - `${CLAUDE_PLUGIN_ROOT}/skills/risk-assessment/SKILL.md`
...
10. **Notion tracking** (if Notion MCP is available):
...
1. Read `${CLAUDE_PLUGIN_ROOT}/teams/config.json`.
...
3. Execute the competing-hypotheses pipeline:
   ...
   **Phase 1 — Parallel Hypothesis Generation** (all 3 scope agents run simultaneously):
   - `scope-agent-a` (`${CLAUDE_PLUGIN_ROOT}/teams/agents/scope-agent-a.md`) — interprets the brief conservatively
   - `scope-agent-b` (`${CLAUDE_PLUGIN_ROOT}/teams/agents/scope-agent-b.md`) — interprets the brief for balanced value
   - `scope-agent-c` (`${CLAUDE_PLUGIN_ROOT}/teams/agents/scope-agent-c.md`) — interprets the brief ambitiously

   **Phase 2 — Parallel Analysis** (both agents evaluate all 3 Phase 1 proposals):
   - `risk-agent` (`${CLAUDE_PLUGIN_ROOT}/teams/agents/risk-agent.md`) — scores risks across all three proposals
   - `pricing-agent` (`${CLAUDE_PLUGIN_ROOT}/teams/agents/pricing-agent.md`) — estimates cost and value for all three proposals

   **Phase 3 — Synthesis**:
   - `sow-lead` (`${CLAUDE_PLUGIN_ROOT}/teams/agents/sow-lead.md`) — synthesizes Phase 1 proposals and Phase 2 analysis into a final SOW document with 3 named client packages and a comparison table
...
```
/sow:from-brief ./briefs/acme-portal-brief.md
/sow:from-brief https://www.notion.so/acme/customer-portal-brief-abc123
/sow:from-brief ./brief.txt --team --output=./proposals/
/sow:from-brief ./brief.md --client="Acme Corp" --budget=75000
/sow:from-brief ./brief.json --weeks=12 --output=./client-sows/
/sow:from-brief                                   # Interactive: asks for file or URL
```
