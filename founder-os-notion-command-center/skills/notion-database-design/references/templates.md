...
Complete schema definitions for the 5 pre-built business templates available via `/notion:template`.
...
When deploying a template via `/notion:template [name]`:
...
4. **Create database** — use the `notion-create-database` tool with all properties defined.
...
When `/notion:template` is called without a name, present:
...
Example: /notion:template "Project Tracker"
```
