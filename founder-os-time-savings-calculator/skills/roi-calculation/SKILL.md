...
Calculate time and money saved by using Founder OS plugins. Accept a DiscoveryResult array from the cross-plugin-discovery skill, apply time estimates per category, and produce structured reports with Mermaid charts. Referenced by: `/savings:weekly`, `/savings:monthly-roi`, `/savings:quick`, `/savings:configure`.

## Purpose and Context

Transform raw plugin task counts into actionable productivity metrics. Receive an array of DiscoveryResult objects (one per plugin category, each containing `category_key`, `completed_count`, `period_start`, `period_end`, and `status`), look up or override the corresponding time estimates, compute per-category and aggregate savings, and render the results as a 5-section Markdown report with embedded Mermaid charts. Use the detailed category reference at `${CLAUDE_PLUGIN_ROOT}/skills/roi-calculation/references/time-estimates.md` for the full breakdown of all 24 category estimates and their rationale. Use `${CLAUDE_PLUGIN_ROOT}/config/task-estimates.json` as the authoritative machine-readable estimates data.
...
For the `/savings:quick` command, produce a condensed single-block output instead of the full 5-section report:
...
- Refer to `${CLAUDE_PLUGIN_ROOT}/skills/roi-calculation/references/time-estimates.md` for the detailed breakdown of all 24 category estimates, their derivation methodology, and customization guidance.
