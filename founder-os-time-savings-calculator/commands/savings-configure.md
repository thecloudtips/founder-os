...
Read the roi-calculation skill at `${CLAUDE_PLUGIN_ROOT}/skills/roi-calculation/SKILL.md` for the configuration resolution priority order and user config schema.
...
Run `/savings:quick` to see your updated calculations.
```
...
```
/savings:configure
/savings:configure --rate=200
/savings:configure --reset
```
