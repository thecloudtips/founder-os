...
Read the slack-analysis skill at `${CLAUDE_PLUGIN_ROOT}/skills/slack-analysis/SKILL.md` for channel scanning, message extraction, thread context resolution, message type classification, and decision detection.

Read the message-prioritization skill at `${CLAUDE_PLUGIN_ROOT}/skills/message-prioritization/SKILL.md` for noise filtering, signal scoring, priority tier assignment, @mention detection, action item extraction, and thread deduplication.
...
```
---
*Digest generated by Slack Digest Engine · [Notion link if saved] · Run `/slack:catch-up` for a quick personal summary*
```
...
**Notion MCP unavailable** (and --output includes notion): Continue with chat output. Append to footer: "Notion unavailable — digest displayed in chat only. Configure Notion MCP per `${CLAUDE_PLUGIN_ROOT}/INSTALL.md`."
...
```
/slack:digest #general #engineering
/slack:digest --all --since=8h
/slack:digest #product #design --since=2d --include-dms
/slack:digest --all --since=2026-03-01 --output=notion
/slack:digest #engineering --output=chat
```
