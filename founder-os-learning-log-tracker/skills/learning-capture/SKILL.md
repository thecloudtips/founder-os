...
Capture daily learnings with auto-generated titles, topic detection, source classification, and related-insight linking. Used by: `/learn:log`.
...
Detect 1-3 topic categories from the 10-category taxonomy. For the full detection algorithm including keyword signals, scoring thresholds, multi-topic handling, and fallback logic, read `${CLAUDE_PLUGIN_ROOT}/skills/learning-capture/references/topic-detection-algorithm.md`.
...
After capturing a new learning, find 2-3 most related past insights from the database. For the full matching algorithm including overlap scoring, recency tiebreaking, keyword fallback, and empty-result handling, read `${CLAUDE_PLUGIN_ROOT}/skills/learning-capture/references/related-insights-algorithm.md`.
...
- **`${CLAUDE_PLUGIN_ROOT}/skills/learning-capture/references/topic-detection-algorithm.md`** — Full 10-category keyword taxonomy with scoring thresholds, multi-topic selection rules, exclusion signals, and source-type fallback logic
- **`${CLAUDE_PLUGIN_ROOT}/skills/learning-capture/references/related-insights-algorithm.md`** — Overlap scoring formula, minimum thresholds, recency tiebreaking, keyword-matching fallback, and empty-result handling
