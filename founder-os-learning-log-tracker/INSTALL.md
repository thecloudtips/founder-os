# Installation: Learning Log Tracker (#29)
...
1. Open Notion and navigate to the page or workspace root where you want the learning log to live
...
> The plugin searches for **[FOS] Learnings** and **[FOS] Weekly Insights** databases first. If not found, it tries **Founder OS HQ - Learnings** and **Founder OS HQ - Weekly Insights**. If still not found, it falls back to **Learning Log Tracker - Learnings** and **Learning Log Tracker - Weekly Insights**. Ensure your Notion workspace has the Founder OS HQ template installed, or create these databases manually and share them with the integration.

---

## Step 4: Configure the MCP Server

The `.mcp.json` file is already included in the plugin directory:
...
Open Claude Code in the `founder-os-learning-log-tracker/` directory and run:

```
/learn:log "Testing my learning log setup — this is my first insight!"
```

Expected result: The plugin will search for **[FOS] Learnings** (preferred), then **Founder OS HQ - Learnings**, then **Learning Log Tracker - Learnings** (fallback) in Notion. If found, it saves your learning with auto-detected topics and a generated title. If no database exists, it will report an error with setup instructions.
...
| `Could not find database` | Integration not shared with parent page, or HQ template not installed | Follow Step 3 — share integration with the target page. Ensure the Founder OS HQ workspace template is installed. |
...
| `Learnings database not found` | HQ template not installed and no legacy database exists | Install the Founder OS HQ workspace template or create the databases manually |
| Database created but empty | First-run expected behavior | Add your first learning with `/learn:log` |
