...
Read the follow-up-detection skill at `${CLAUDE_PLUGIN_ROOT}/skills/follow-up-detection/SKILL.md` for thread context analysis and priority-based scheduling logic.
...
- `--all` (optional) — create reminders for all pending follow-ups from the last `/followup:check` run. Mutually exclusive with `$1`.
...
1. Check if `/followup:check` was run recently in this conversation. If follow-up results are available, present them as a numbered list and ask the user to pick one or confirm `--all`.
2. Otherwise, prompt: "No email specified. Run `/followup:check` first, or provide a thread ID, subject keyword, or `--all` flag."
...
3. **Create Calendar event**:
   ...
   - **Description**: Include thread context:
     ...
     Thread ID: [thread_id]
     Tip: Run /followup:nudge [thread_id] to draft a follow-up email.
     ```
...
1. **Gather pending follow-ups**: Use results from the most recent `/followup:check` in this conversation, or run a fresh scan if no recent results exist. Include only items with Status "Awaiting Reply" and priority >= 2 (skip priority 1 "monitor only" items).
...
- Suggest: "gws CLI (Calendar) unavailable. Add these reminders manually, or configure Google Calendar per `${CLAUDE_PLUGIN_ROOT}/INSTALL.md`."
...
Tip: When the reminder fires, run `/followup:nudge [thread_id]` to draft a follow-up email.
```
...
- Display: "No pending follow-ups with priority >= 2. Run `/followup:check` to scan for new follow-ups."
...
```
/followup:remind 18e3a4b2c1d0e5f6
/followup:remind "project proposal"
/followup:remind 18e3a4b2c1d0e5f6 --in=2d
/followup:remind --all
/followup:remind --all --in=1d
```
