...
Read the contract-analysis skill at `${CLAUDE_PLUGIN_ROOT}/skills/contract-analysis/SKILL.md` for contract structure recognition, file format handling, contract type detection, key term extraction patterns, output structure, and Notion integration.

Read the legal-risk-detection skill at `${CLAUDE_PLUGIN_ROOT}/skills/legal-risk-detection/SKILL.md` for RAG classification system, risk categories, risk flag taxonomy, overall risk level calculation, and output structure.
...
5. **Extract key terms**: For each of the 7 categories (Payment, Duration/Renewal, IP, Confidentiality, Liability, Termination, Warranty), extract specific terms. Consult `${CLAUDE_PLUGIN_ROOT}/skills/contract-analysis/references/clause-patterns.md` for detailed extraction patterns and examples.
...
7. **Detect risks**: Apply the legal-risk-detection skill to evaluate each extracted term and clause. Assign RAG severity (Red/Yellow/Green) to each finding. Consult `${CLAUDE_PLUGIN_ROOT}/skills/legal-risk-detection/references/risk-patterns.md` for the comprehensive risk pattern library.
...
```
/contract:analyze path/to/contract.pdf
/contract:analyze ~/Documents/service-agreement.docx
/contract:analyze contracts/nda-draft.md
