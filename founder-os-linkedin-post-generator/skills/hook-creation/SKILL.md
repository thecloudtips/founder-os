---
name: Hook Creation
description: "Crafts high-impact opening lines for LinkedIn posts designed to stop the scroll. Activates when the user wants a stronger hook, better opening line, or asks 'how should I start this LinkedIn post?' Covers proven hook formulas, curiosity gaps, and pattern-interrupt techniques that drive engagement."
globs:
...
Craft high-impact opening lines for LinkedIn posts that stop the scroll, survive the "see more" fold, and compel readers to expand the full post. Used by: `/linkedin:post` (full post generation) and `/linkedin:variations` (hook A/B testing and iteration).
...
For detailed hook examples organized by formula type and audience segment, see `${CLAUDE_PLUGIN_ROOT}/skills/hook-creation/references/hook-templates.md`.
