...
| In Progress | On Hold | Manual via `/goal:update --status=on-hold` |
| In Progress | Completed | Progress reaches 100%, all milestones Done, or manual close via `/goal:close` |
| On Hold | In Progress | Manual resume via `/goal:update --status=in-progress` |
| Any | Archived | Manual via `/goal:close --archive` |
...
Reference `${CLAUDE_PLUGIN_ROOT}/skills/goal-tracking/references/status-lifecycle.md` for the full transition matrix, guard conditions, and error messages for each invalid transition.
...
Reference `${CLAUDE_PLUGIN_ROOT}/skills/goal-tracking/references/milestone-progress-formula.md` for edge cases, division-by-zero handling, and worked examples.
...
- Accept manual progress values via `/goal:update --progress=N`.
...
When the user attempts to close a goal via `/goal:close` and one or more milestones remain with Status = Not Started or In Progress:
...
- `${CLAUDE_PLUGIN_ROOT}/skills/goal-tracking/references/milestone-progress-formula.md` — Full formula specification with division-by-zero guards, partial credit rules, rounding behavior, and step-by-step worked examples for common and edge-case scenarios.
- `${CLAUDE_PLUGIN_ROOT}/skills/goal-tracking/references/status-lifecycle.md` — Complete transition matrix covering all valid and invalid status changes, guard conditions for each transition, auto-trigger rules, and error messages to display for blocked transitions.
