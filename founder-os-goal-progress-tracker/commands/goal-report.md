...
3. `${CLAUDE_PLUGIN_ROOT}/skills/goal-reporting/SKILL.md`
...
- No positional arguments. If arguments provided that look like a goal name, suggest /goal:check instead.
...
5. **Render Dashboard Table**: Build table per dashboard-table-format specification. Read `${CLAUDE_PLUGIN_ROOT}/skills/goal-reporting/references/dashboard-table-format.md` for full formatting rules. Sort by RAG severity.
...
8. **Generate Gantt Timeline**: Build Mermaid gantt chart per gantt-generation specification. Read `${CLAUDE_PLUGIN_ROOT}/skills/goal-reporting/references/gantt-generation.md` for syntax. Group by category, apply color directives.
...
Generated from N goals | Use /goal:check [name] for detail
```
...
/goal:report --category=Product --output=file --path=q2-goals.md
/goal:report --output=notion
```
