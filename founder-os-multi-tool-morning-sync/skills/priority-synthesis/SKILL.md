...
Assemble briefing content in this structure for `/morning:sync` Notion output. The briefing is stored in the "[FOS] Briefings" database with Type = "Morning Sync" (falls back to "Founder OS HQ - Briefings", then "Morning Sync - Briefings"). Use Heading 2 blocks for section headers with divider blocks between sections. Omit sections where the source was unavailable, except Top Priorities and Quick Stats (always present).
...
For `/morning:quick`, produce a compact chat-only format. No Notion page. Designed to be read in under 30 seconds. Maximum 15 lines.
