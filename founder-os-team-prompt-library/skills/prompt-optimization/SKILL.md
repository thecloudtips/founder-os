...
Apply these strategies during active rewrite mode (`/prompt:optimize`). Select the strategies that address the lowest-scoring dimensions. Apply up to 3 strategies per rewrite to avoid over-engineering.
...
REWRITTEN PROMPT: [Only present in /prompt:optimize mode]
...
In passive quality-check mode (called from `/prompt:add`):
...
- If the score is 10 or below, recommend running `/prompt:optimize` before saving and ask whether to proceed.

In active rewrite mode (called from `/prompt:optimize`):
...
For detailed scoring examples showing before/after prompts at each score level (1–5) across all dimensions, and worked rewrite walkthroughs, see `${CLAUDE_PLUGIN_ROOT}/skills/prompt-optimization/references/scoring-rubric.md`.
