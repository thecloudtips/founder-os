...
- **Option A — Existing draft in conversation**: If a LinkedIn post was generated earlier in this conversation (via `/founder-os:linkedin:post` or `/founder-os:linkedin:from-doc`), use that as the source. The post text and its metadata (topic, framework, audience) should be available from the prior output.
...
- **No input**: If no arguments AND no prior post in conversation, prompt: "Paste a LinkedIn post draft, or provide a topic. You can also run `/founder-os:linkedin:post [topic]` first, then run `/founder-os:linkedin:variations` to get variations."  Then stop and wait.
...
This command is ephemeral by design (like P19 `/founder-os:slack:catch-up`):
- No file output
- No Notion logging
...
```
/founder-os:linkedin:variations
/founder-os:linkedin:variations "why I stopped using OKRs"
/founder-os:linkedin:variations --count=5
/founder-os:linkedin:variations --audience=technical
/founder-os:linkedin:variations "Paste your draft here\nLine 2\nLine 3" --count=4
```
