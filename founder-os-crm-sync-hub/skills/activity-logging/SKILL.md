...
For the full list of optional properties beyond this core set, see `${CLAUDE_PLUGIN_ROOT}/skills/activity-logging/references/activity-schema.md`.
...
For detailed signal word lists and classification examples for each activity type, see `${CLAUDE_PLUGIN_ROOT}/skills/activity-logging/references/activity-schema.md`.
...
For the detailed activity schema with all optional fields, full summary templates with examples for each activity type, sentiment signal word lists, title generation rules, and error recovery procedures, consult `${CLAUDE_PLUGIN_ROOT}/skills/activity-logging/references/activity-schema.md`.
