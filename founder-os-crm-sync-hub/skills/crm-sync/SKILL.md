...
### Email Sync (`/crm:sync-email [thread_id]`)
...
### Meeting Sync (`/crm:sync-meeting [event_id]`)
...
1. **Scan**: Fetch recent items within the lookback window (default: 7 days, configurable via the `--since` flag). For parsing rules on the `--since` flag format, see `${CLAUDE_PLUGIN_ROOT}/skills/crm-sync/references/sync-patterns.md`.
...
## Context Command Workflow (`/crm:context [client]`)
...
3. **Display**: Format as a concise CRM context view. This is intentionally lighter than P20's `/client:load` dossier -- focus on structured CRM data rather than cross-source intelligence gathering. For the detailed output format template, see `${CLAUDE_PLUGIN_ROOT}/skills/crm-sync/references/sync-patterns.md`.
...
- **Rate limiting**: If Notion returns a 429 (rate limit) response, pause for 5 seconds and retry. If rate limiting persists across 3 consecutive requests, pause for 30 seconds before resuming. See `${CLAUDE_PLUGIN_ROOT}/skills/crm-sync/references/sync-patterns.md` for detailed rate limiting patterns.
...
- **Both Gmail and Calendar unavailable**: Only the `/crm:context` command remains functional (reading from Notion only). Report the limitation.
...
For advanced batch processing patterns, rate limiting strategies, `--since` flag parsing rules, CRM context view format template, cross-plugin integration notes, and batch filter rules, consult `${CLAUDE_PLUGIN_ROOT}/skills/crm-sync/references/sync-patterns.md`.
