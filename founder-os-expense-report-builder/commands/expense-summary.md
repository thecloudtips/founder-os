...
Ephemeral spending overview for a given period. Output directly to chat — no file saving, no Notion DB write. This is the lightweight counterpart to `/expense:report`, optimized for a quick "how much did I spend?" answer.

## Load Skills

Read the expense-reporting skill at `${CLAUDE_PLUGIN_ROOT}/skills/expense-reporting/SKILL.md` for date range parsing rules, the data aggregation pipeline, and the P11 category taxonomy.

Read the expense-categorization skill at `${CLAUDE_PLUGIN_ROOT}/skills/expense-categorization/SKILL.md` for the 14-category taxonomy, classification signals, and confidence scoring — needed when local files lack pre-assigned categories.
...
No expenses found for [Period Label]. Run `/invoice:process` to import invoices, or check that the date range is correct.
```
...
This command does NOT save output to a file or write to a Notion database. It is ephemeral by design. For a full 7-section report with file output and Notion logging, use `/expense:report`.
...
**Mixed currencies**: Report amounts in the majority currency. Add a note: "_[N] entries in other currencies excluded -- run `/expense:report` for full multi-currency handling_".

## Usage Examples

```
/expense:summary
/expense:summary 2024-03
/expense:summary Q1 2024
/expense:summary "last month"
/expense:summary "this quarter" --top=5
/expense:summary "last quarter" --top=20
```
