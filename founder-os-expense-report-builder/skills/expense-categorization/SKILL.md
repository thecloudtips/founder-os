...
Assign standard expense categories to individual receipts and invoice files when building an expense report. Each file processed by `/expense:report` receives a category, budget code, tax-deductibility flag, and confidence score before aggregation into the final report.
