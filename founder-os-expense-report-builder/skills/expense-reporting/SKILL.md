...
3. For image and PDF files, note the file path and flag as "unprocessed -- run /invoice:process first" unless a matching record exists in the Notion data from Step 1.
...
Consult `${CLAUDE_PLUGIN_ROOT}/skills/expense-reporting/references/report-sections.md` for detailed section templates, example output, table formats, and column calculation formulas.
