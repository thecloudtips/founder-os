...
Read: `${CLAUDE_PLUGIN_ROOT}/skills/workflow-execution/SKILL.md`
...
1. **Check Notion**: Verify Notion MCP is available. If unavailable:
   ...
   Run /workflow:run to execute workflows — results display in chat regardless of Notion status.
   ```

2. **Query Executions DB**: Search for a database titled "[FOS] Workflows" first. If not found, try "Founder OS HQ - Workflows". If not found, fall back to "Workflow Automator - Executions" (legacy). If no DB exists, display: "No execution history yet. Run a workflow with /workflow:run first." When querying the consolidated DB, filter by Type="Execution" to exclude SOP records.
...
/workflow:status --status=failed --last=10
/workflow:status weekly-review --last=3
```
