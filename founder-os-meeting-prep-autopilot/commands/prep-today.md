...
Generate meeting prep documents for every qualifying calendar meeting today. Fetch all events, filter out non-meetings, then run the full `/meeting:prep` logic for each meeting in chronological order. Save each prep to Notion and display a summary table at the end with links to all generated prep pages.
...
1. Read the meeting-context skill at `${CLAUDE_PLUGIN_ROOT}/skills/meeting-context/SKILL.md` for event identity resolution, meeting classification, attendee context lookup, event filtering rules, and graceful degradation rules.
2. Read the talking-points skill at `${CLAUDE_PLUGIN_ROOT}/skills/talking-points/SKILL.md` for framework selection, talking point generation, open/close structure, and context-aware customization.
3. If `--team` is present, read the pipeline configuration at `${CLAUDE_PLUGIN_ROOT}/teams/config.json`.
...
2. Execute the full `/meeting:prep` logic for this single meeting:
   ...
   - **If `--team` IS present**: Launch the parallel-gathering pipeline per `${CLAUDE_PLUGIN_ROOT}/teams/config.json`. Pass cached attendee data to the gatherer agents to avoid redundant lookups. Agents read their definitions from `${CLAUDE_PLUGIN_ROOT}/teams/agents/[agent-name].md`.
3. **Save to Notion**:
...
- **Calendar returns events but all are filtered out**: Display: "Found [N] calendar events but all were filtered out. Reasons: [breakdown]. Use `/meeting:prep [event-title]` to prep a specific event that was filtered."
...
```
/meeting:prep-today                           # Prep all today's meetings, single-agent, with confirmation
/meeting:prep-today --yes                     # Prep all today's meetings, auto-proceed
/meeting:prep-today --team                    # Full parallel pipeline for each meeting
/meeting:prep-today --skip-internal           # Skip internal syncs and group meetings
/meeting:prep-today --team --skip-internal    # Team pipeline, external/important meetings only
/meeting:prep-today --hours=720 --yes         # 30-day email lookback, auto-proceed
/meeting:prep-today --team --skip-internal --hours=2160 --yes  # Full pipeline, externals only, 90-day lookback, no confirmation
```
