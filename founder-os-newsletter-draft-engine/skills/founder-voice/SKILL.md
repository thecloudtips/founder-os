...
Apply a consistent, opinionated writing voice to newsletter content. Used by: `/newsletter:draft` and `/newsletter` commands when composing or editing newsletter sections. This skill defines the tone, rhythm, opinion style, and structural patterns that distinguish a founder-written newsletter from generic content marketing.
...
Refer to `${CLAUDE_PLUGIN_ROOT}/skills/founder-voice/references/voice-examples.md` for detailed before-and-after rewrite examples demonstrating how to transform generic content marketing into founder voice.
