...
Read the topic-research skill at `${CLAUDE_PLUGIN_ROOT}/skills/topic-research/SKILL.md` for query formulation patterns, source type taxonomy, scoring formulas, deduplication logic, and finding extraction rules.
...
Run `/newsletter:outline` to structure these findings into a newsletter.
```
...
```
/newsletter:research AI agents
/newsletter:research "Claude MCP plugins" --days=7
/newsletter:research LLM fine-tuning --sources=web,github
/newsletter:research "React Server Components" --sources=web,reddit --days=30
/newsletter:research Kubernetes --sources=github
```
