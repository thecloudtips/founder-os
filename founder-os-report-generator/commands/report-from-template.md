...
1. Read all 5 skills from `${CLAUDE_PLUGIN_ROOT}/skills/*/SKILL.md`.
...
2. Read `${CLAUDE_PLUGIN_ROOT}/teams/config.json` and execute the 5-agent pipeline.
...
```
/report:from-template                                           # Interactive: list templates, ask for choice
/report:from-template --template=executive-summary --data=q4.csv
/report:from-template --template=full-business-report --data=./data/ --team
/report:from-template --template=project-status-report --data=sprint-metrics.json --output=./status/
```
