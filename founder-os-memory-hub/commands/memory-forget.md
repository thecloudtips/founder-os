...
Run: /memory:forget acme-vip-flag
```

If no matches at all: output "Memory '[key]' not found. Use /memory:list to browse all stored memories." and stop.
...
If the Notion MCP is unavailable or the page is not found, skip silently and note `notion_sync: "unavailable"` in output. This is a non-blocking step.
...
This behavior will no longer apply. To re-teach it, use /memory:teach.
```
...
| Key not found, no fuzzy matches | Report not-found message, suggest `/memory:list` |
...
| Notion MCP unavailable | Skip Notion archive step silently, note in output |
| Deletion verification fails | Report SQLite error with raw message; do not claim success |

## Usage Examples

```
/memory:forget acme-vip-flag            # Look up, show details, confirm before deleting
/memory:forget acme-vip-flag --force    # Delete immediately, no confirmation prompt
/memory:forget acme-vip                 # Key not found → suggest acme-vip-flag etc.
```
