...
2. `${CLAUDE_PLUGIN_ROOT}/skills/notion-sync/SKILL.md` — Notion field mapping, conflict rules, rate-limit handling
...
- If Notion MCP is not configured or unreachable: halt with message `"Notion MCP unavailable — cannot sync. Check .mcp.json configuration."`
- If local memory store does not exist and cannot be initialized: halt with message `"Memory store not found. Run /memory:store first to initialize."`
...
/memory:sync --direction both --dry-run
/memory:sync --dry-run
```
