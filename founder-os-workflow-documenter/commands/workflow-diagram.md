...
Generate a quick Mermaid flowchart from a workflow description or an existing documented SOP. This is a lightweight companion to `/workflow:document` — it produces a visual diagram without the full 7-section SOP pipeline. Ephemeral by default (chat only). Never writes to Notion.
...
If the workflow needed consolidation to fit 25 nodes, note: "Consolidated from [original_count] steps to fit diagram limits. Run `/workflow:document` for the full SOP."
...
- Running `/workflow:document` for guided SOP creation with step-by-step prompts"
