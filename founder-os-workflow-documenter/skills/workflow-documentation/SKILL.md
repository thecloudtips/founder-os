...
Parse workflow descriptions from any input format into structured, standardized process documents with discrete steps, identified tools, detected handoffs, and scored complexity. Used by: `/workflow:document` (full SOP generation) and `/workflow:diagram` (visual workflow rendering).
...
The command layer consumes this schema to render the appropriate output: `/workflow:document` produces a formatted SOP markdown document; `/workflow:diagram` produces a Mermaid flowchart. This skill produces the data; the commands produce the presentation.
