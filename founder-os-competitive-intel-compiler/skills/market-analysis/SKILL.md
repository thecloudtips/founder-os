...
Present SWOT as a 2x2 matrix summary followed by bulleted details. Consult `${CLAUDE_PLUGIN_ROOT}/skills/market-analysis/references/analysis-frameworks.md` for SWOT templates and inference rules.
...
## Matrix Building (for /compete:matrix)
...
When called after `/compete:research` or with structured competitor data, follow this sequence:
...
4. **Run SWOT per competitor**: Apply inference rules from `${CLAUDE_PLUGIN_ROOT}/skills/market-analysis/references/analysis-frameworks.md`
...
8. **Self-comparison (if --your-product provided)**: Run feature gap analysis and differentiation mapping — see `${CLAUDE_PLUGIN_ROOT}/skills/market-analysis/references/analysis-frameworks.md` Section 5 for step-by-step process
