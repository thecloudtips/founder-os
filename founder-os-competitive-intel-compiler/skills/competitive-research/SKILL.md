...
Consult `${CLAUDE_PLUGIN_ROOT}/skills/competitive-research/references/query-patterns.md` for full templated queries for each dimension, including site-specific variants and matrix query patterns for multi-competitor research.
...
When running `/compete:matrix`, produce one `competitor_data` object per company and pass the full array to the matrix formatter.
