...
Read the weekly-reflection skill at `${CLAUDE_PLUGIN_ROOT}/skills/weekly-reflection/SKILL.md` for the complete review pipeline: week boundary rules, database auto-discovery algorithm, 3-source data gathering, blocker detection signals, priority ranking algorithm, and Notion output formatting.
...
Read `${CLAUDE_PLUGIN_ROOT}/skills/weekly-reflection/references/review-structure.md` for detailed section templates. Assemble the review with exactly 6 sections:
...
```
/weekly:review                           # Review the most recent completed week
/weekly:review --date=2026-02-24         # Review the week containing Feb 24
/weekly:review --output=chat             # Display review in chat instead of Notion
/weekly:review --output=both             # Both Notion page and chat display
/weekly:review --date=2026-03-03 --output=notion  # Specific week, Notion output
```
